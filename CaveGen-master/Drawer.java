...
public class Drawer {
    ...
    Image getTeki(Teki t, float scale) throws Exception {
        ...
        String loc = "files/" + CaveGen.fileSystem + "/" + "enemytex/tekiIcons/" + name + ".png";
        if (CaveGen.colossal) loc = "files/" + "gc" + "/" + "enemytex/tekiIcons/" + name + ".png";
    ...
    Image getItem(Item t, String inside, String region, float scale) throws Exception {
        ...
        String loc = "files/" + CaveGen.fileSystem + "/" + "resulttex/itemIcons/" + name.toLowerCase() + ".png";
    ...
    void draw(CaveGen g, boolean drawAsReport, Aggregator aggregator) throws Exception {
        ...
        if (CaveGen.drawPretty) {
            if (g.skyboxFile.equals("flooring")) {
        ...
        if (drawAsReport) {
            ...
            if (g.isFinalFloor || g.hasGeyser) {
                ...
                if (g.challengeMode) G.drawImage(getSpecial("clog"), x+90, z-15, null);
            }
            int xx = 0;
            int zzd = 20;
            z += 5;
            ...
            int ext = 4;
            G.setFont(new Font("Serif", Font.BOLD, 36));
            drawTextOutline(G, "Teki (x" + g.maxMainTeki + ")", x, z, colorsSP[1], bgt);
            ...
            for (int type: types) {
                ...
                for (Teki t: li) {
                    if (t.type == type) {
                        ...
                        if (type == 6)
                            drawTextOutline(G, "  x" + t.min, xx, z+8+ext, new Color(0,150,0), bgt);
                        else if (t.weight == 0)
                            drawTextOutline(G, "  x" + t.min, xx, z+8+ext, colorsSP[type], bgt);
                        else if (t.min == 0)
                            drawTextOutline(G, "  %" + t.weight, xx, z+8+ext, colorsSP[type], bgt);
                        else
                            drawTextOutline(G, "x" + t.min + "%" + t.weight, xx, z+8+ext, colorsSP[type], bgt);
                        xx += spa;
                    }
                }
            }
            z += zz + zzd;
            G.setFont(new Font("Serif", Font.BOLD, 36));
            drawTextOutline(G, "Item (x" + g.maxItem + ")", x, z, colorsSP[2], bgt);
            G.setFont(new Font("Serif", Font.BOLD, 16));
            xx = 170;
            for (Item t: g.spawnItem) {
                ...
                if (t.weight == 0)
                    drawTextOutline(G, "  x" + t.min, xx, z+8+ext, colorsSP[2], bgt);
                else if (t.min == 0)
                    drawTextOutline(G, "  %" + t.weight, xx, z+8+ext, colorsSP[2], bgt);
                else
                    drawTextOutline(G, "x" + t.min + "%" + t.weight, xx, z+8+ext, colorsSP[2], bgt);
                xx += spa;
            }
            z += zz + zzd;
            G.setFont(new Font("Serif", Font.BOLD, 36));
            ...
            for (Teki t: g.spawnCapTeki) {
                drawTeki(G,g,t,xx,z-40);
                if (t.weight == 0)
                    drawTextOutline(G,"  x" + t.min, xx, z+8+ext, colorsSP[t.type==0?0:9], bgt);
                else if (t.min == 0)
                    drawTextOutline(G,"  %" + t.weight, xx, z+8+ext, colorsSP[t.type==0?0:9], bgt);
                else
                    drawTextOutline(G,"x" + t.min + "%" + t.weight, xx, z+8+ext, colorsSP[t.type==0?0:9], bgt);
                xx += spa;
            }
            ...
            z += zz + zzd;
            G.setFont(new Font("Serif", Font.BOLD, 36));
            drawTextOutline(G, "Gate (x" + g.maxGate + ")", x, z, colorsSP[5], bgt);
            G.setFont(new Font("Serif", Font.BOLD, 16));
            xx = 170;
            for (Gate t: g.spawnGate) {
                ...
                drawTextOutline(G, "%" + t.weight, xx+50, z+8+ext, colorsSP[5], bgt);
                xx += spa*2;
            }
            bgt = new Color(0,0,0);
        }
        ...
        if (g.drawTreasureGauge && !g.challengeMode) {
            ...
            for (Teki t: g.placedTekis) {
                if (t.itemInside != null &&
                    (g.isFinalFloor || !g.specialCaveInfoName.equals("SC")
            ...
            if (!g.drawNoTeki) {
                for (Teki t: g.placedTekis) {
                    ...
                    try {
                        ...
                        if (t.tekiName.equalsIgnoreCase("blackman") && !g.isFinalFloor) {
        ...
        if (aggregator != null) {
            ...
            outputDir3.mkdir();
            File outputDir4 = new File(output + as + "/" + "Output" + "/");
            outputDir4.mkdir();
            File outputFile3 = new File(output + as + "/" + "Output" + "/output.png");
        ...
        if (true) {
            ...
            outputDir.mkdir();
            File outputFile = new File(output + "/Output/output.png");
